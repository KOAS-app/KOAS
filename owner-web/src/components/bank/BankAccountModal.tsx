...
export default function BankAccountModal({ account, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getApiError(err, `Failed to ${isEdit ? 'update' : 'add'} bank account.`));
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--color-border)]">
          <div>
            ...
            <p className="text-[0.8125rem] text-[var(--color-text-muted)] mt-0.5">
              {isEdit ? 'Update your bank account details.' : 'Add a new bank account for payments.'}
            </p>
          </div>
          <button
            onClick={onClose}
